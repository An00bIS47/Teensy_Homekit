// 
// HAPFakeGatoScheduleEnergyOLD.cpp
// Homekit
//
...
FLASHMEM 
#endif
HAPFakeGatoScheduleEnergyOLD::HAPFakeGatoScheduleEnergyOLD(){
	_statusLED = 0x00;
...
FLASHMEM 
#endif
HAPFakeGatoScheduleEnergyOLD::HAPFakeGatoScheduleEnergyOLD(String serialNumber, std::function<void(uint16_t)> callbackStartTimer, std::function<void(uint16_t)> callbackEndTimer, std::function<uint32_t(void)> callbackRefTime, std::function<uint32_t(void)> callbackTimestampLastActivity, std::function<uint32_t(void)> callbackTimestampLastEntry){
...
FLASHMEM 
#endif
void HAPFakeGatoScheduleEnergyOLD::begin(){

}


bool HAPFakeGatoScheduleEnergyOLD::decodeToggleOnOff(uint8_t* data){
	return data[1] & 0x01;
}

void HAPFakeGatoScheduleEnergyOLD::enable(bool on){		
	_timers.enable(on);
}

void HAPFakeGatoScheduleEnergyOLD::setStatusLED(uint8_t mode){	
	_statusLED = mode;
}

bool HAPFakeGatoScheduleEnergyOLD::isEnabled(){
	return _timers.isEnabled();
}

void HAPFakeGatoScheduleEnergyOLD::decodeDays(uint8_t *data){
 ...
 * @param data 
 */
void HAPFakeGatoScheduleEnergyOLD::decodePrograms(uint8_t* data){
...
uint32_t HAPFakeGatoScheduleEnergyOLD::encodeTimerCount(uint8_t timerCount){
	uint32_t result = 0;
	if (timerCount < 3) {
...
uint8_t HAPFakeGatoScheduleEnergyOLD::encodeProgramCount(uint8_t programCount){
 ...
 */
void HAPFakeGatoScheduleEnergyOLD::encodePrograms(uint8_t* data, size_t *dataSize){
...
void HAPFakeGatoScheduleEnergyOLD::clear(){
	for (int i = 0; i < _programEvents.size(); i++){
...
FLASHMEM 
#endif
void HAPFakeGatoScheduleEnergyOLD::fromJson(JsonObject &root){
...
FLASHMEM 
#endif
JsonObject HAPFakeGatoScheduleEnergyOLD::toJson(){
	/*
		"days": uint32_t,
		"programs": "0503....."
	*/
...
String HAPFakeGatoScheduleEnergyOLD::buildScheduleString(){
    TLV8 tlv;
...
void HAPFakeGatoScheduleEnergyOLD::programTimers() {
	...
	for (size_t i = 0; i < _programEvents.size(); i++){
		...
		for (size_t j = 0; j < _programEvents[i].timerEvents.size(); j++){
			...
			HAPDailyTimer timer(_programEvents[i].timerEvents[j].hour,
								...
								daysMask,
								FIXED,
								std::bind(&HAPFakeGatoScheduleEnergyOLD::callbackTimerStart, this, std::placeholders::_1),
...
FLASHMEM 
#endif
void HAPFakeGatoScheduleEnergyOLD::callbackTimerStart(uint16_t state){
#if HAP_DEBUG_FAKEGATO_SCHEDULE
	LogI(HAPTime::timeString() + " " + "HAPFakeGatoScheduleEnergyOLD" + "->" + String(__FUNCTION__) + " [   ] " + "Timed action: START", true);
#endif	
...
FLASHMEM 
#endif
void HAPFakeGatoScheduleEnergyOLD::callbackTimerEnd(uint16_t state){
#if HAP_DEBUG_FAKEGATO_SCHEDULE	
	LogI(HAPTime::timeString() + " " + "HAPFakeGatoScheduleEnergyOLD" + "->" + String(__FUNCTION__) + " [   ] " + "Timed action: END", true);
#endif	
