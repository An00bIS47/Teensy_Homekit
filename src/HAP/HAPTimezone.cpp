 ...
 *----------------------------------------------------------------------*/
HAPTimezone::HAPTimezone(HAPTimeChangeRule dstStart, HAPTimeChangeRule stdStart)
 ...
 * daylight time.                                                       *
 *----------------------------------------------------------------------*/
HAPTimezone::HAPTimezone(HAPTimeChangeRule stdTime)
 ...
 *----------------------------------------------------------------------*/
HAPTimezone::HAPTimezone(int address)
{
    readRules(address);
}
#endif
 ...
 *----------------------------------------------------------------------*/
time_t HAPTimezone::toLocal(time_t utc)
{
 ...
 *----------------------------------------------------------------------*/
time_t HAPTimezone::toLocal(time_t utc, HAPTimeChangeRule **tcr)
{
    // recalculate the time change points if needed
    if (year(utc) != year(m_dstUTC)) calcTimeChanges(year(utc));
 ...
 * should be avoided!                                                   *
 *----------------------------------------------------------------------*/
time_t HAPTimezone::toUTC(time_t local)
{
    // recalculate the time change points if needed
 ...
 *----------------------------------------------------------------------*/
bool HAPTimezone::utcIsDST(time_t utc)
{
    // recalculate the time change points if needed
    if (year(utc) != year(m_dstUTC)) calcTimeChanges(year(utc));
 ...
 * or the Standard time interval.                                       *
 *----------------------------------------------------------------------*/
bool HAPTimezone::locIsDST(time_t local)
{
    // recalculate the time change points if needed
    if (year(local) != year(m_dstLoc)) calcTimeChanges(year(local));
 ...
 *----------------------------------------------------------------------*/
void HAPTimezone::calcTimeChanges(int yr)
{
 ...
 *----------------------------------------------------------------------*/
void HAPTimezone::initTimeChanges()
{
    m_dstLoc = 0;
    m_stdLoc = 0;
    m_dstUTC = 0;
 ...
 * for the given year.                                                  *
 *----------------------------------------------------------------------*/
time_t HAPTimezone::toTime_t(HAPTimeChangeRule r, int yr)
{
 ...
 *----------------------------------------------------------------------*/
void HAPTimezone::setRules(HAPTimeChangeRule dstStart, HAPTimeChangeRule stdStart)
{
    m_dst = dstStart;
    m_std = stdStart;
...
time_t HAPTimezone::locDSTStart(time_t local) {
    if (year(local) != year(m_dstLoc)) calcTimeChanges(year(local));
    return toTime_t(m_dst, year(local));
}

time_t HAPTimezone::locSTDStart(time_t local){
    if (year(local) != year(m_dstLoc)) calcTimeChanges(year(local));
    return toTime_t(m_std, year(local));
}

int HAPTimezone::offset(time_t local){
    if (year(local) != year(m_dstLoc)) calcTimeChanges(year(local));

    if (locIsDST(local)) return m_dstLoc.offset;
    return m_stdLoc.offset;
}
