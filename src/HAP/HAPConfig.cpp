...
void HAPConfig::begin(){
    ...
    for(std::vector<String>::iterator it = names.begin(); it != names.end(); ++it) {
        ...
        Serial.println("   - Loading " + plugin->name());      
...
#elif defined( CORE_TEENSY)  
    EepromStream eepromStream(HAP_EEPROM_CONFIG_OFFSET, HAP_EEPROM_CONFIG_SIZE);
    deserializeJson(_config, eepromStream);
#endif
...
#elif defined( CORE_TEENSY)    
    EepromStream eepromStream(HAP_EEPROM_CONFIG_OFFSET, HAP_EEPROM_CONFIG_SIZE);
    serializeJson(_config, eepromStream);
#endif
