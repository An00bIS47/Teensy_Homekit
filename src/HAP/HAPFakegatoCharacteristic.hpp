...
public:
    // HAPFakegatoCharacteristicBase(std::function<T(void)> callback) : HAPFakegatoCharacteristic(HAPFakegatoSignature_Temperature, sizeof(T)) {
...
protected:    
    uint8_t _size;
    std::function<T(void)> _valueGetFunctionCall;
};
